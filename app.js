...
const demoArtists = ['Frank Ocean','Lana Del Rey','Tame Impala','SZA','The Weeknd','BLACKPINK','Sabrina Carpenter','Taylor Swift','Billie Eilish','Arctic Monkeys','Bruno Mars','Bad Bunny','Feid','Peso Pluma','Tyler, The Creator','Kendrick Lamar','Olivia Rodrigo','Charli xcx','Dua Lipa','The Neighbourhood','Daniel Caesar','Cigarettes After Sex','Clairo','Wave to Earth','NewJeans','ROSÉ','Chappell Roan'];
let selectedGenres = JSON.parse(localStorage.getItem('naevis_genres') || '[]');
let selectedArtists = JSON.parse(localStorage.getItem('naevis_artists') || '[]');
...
function renderGenres(filter = '') {
  ...
  const list = genres.filter(x => !q || x.toLowerCase().includes(q));
  g.innerHTML = list.map(x => {
    const selected = selectedGenres.includes(x);
    return `<button class="retro genre-chip ${selected ? 'blue selected' : ''}" type="button" data-genre="${escapeHtml(x)}">${escapeHtml(x)}</button>`;
  }).join('') || '<small>No genres found. Try another word.</small>';
  $$('.genre-chip').forEach(btn => btn.onclick = () => {
    const value = btn.dataset.genre;
    selectedGenres = selectedGenres.includes(value) ? selectedGenres.filter(x => x !== value) : [...selectedGenres, value];
    localStorage.setItem('naevis_genres', JSON.stringify(selectedGenres));
...
function renderArtists(list = demoArtists) {
  ...
  box.innerHTML = list.map(x => {
    const selected = selectedArtists.includes(x);
    return `<button class="artist ${selected ? 'selected' : ''}" type="button" data-name="${escapeHtml(x)}"><div class="artist-art">🎵</div><b>${escapeHtml(x)}</b><small>${selected ? '✓ Added' : 'Add artist'}</small></button>`;
  }).join('') || '<small>No artists found.</small>';
  $$('.artist').forEach(a => a.onclick = () => {
    const value = a.dataset.name;
    selectedArtists = selectedArtists.includes(value) ? selectedArtists.filter(x => x !== value) : [...selectedArtists, value];
    localStorage.setItem('naevis_artists', JSON.stringify(selectedArtists));
    renderArtists(list);
    updateMusicSummary();
  });
}
...
function updateMusicSummary() {
  const g = $('#selectedGenres');
  const a = $('#selectedArtists');
  if (g) g.innerHTML = selectedGenres.length ? selectedGenres.map(x => `<span>${escapeHtml(x)}</span>`).join('') : '<small>No genres selected yet.</small>';
  if (a) a.innerHTML = selectedArtists.length ? selectedArtists.map(x => `<span>${escapeHtml(x)}</span>`).join('') : '<small>No artists selected yet.</small>';
}
...
async function searchSpotify() {
  ...
  if (!q) return renderArtists(demoArtists);
  try {
    const token = await validToken();
    if (token) {
      const response = await spotifyFetch('https://api.spotify.com/v1/search?' + new URLSearchParams({q,type:'artist',limit:12,market:'MX'}));
      if (response.ok) {
        const data = await response.json();
        const list = (data.artists?.items || []).map(a => a.name);
        return renderArtists(list.length ? list : []);
      }
    }
    // Before Spotify is connected, NAEVIS still lets the user choose from a local starter catalog.
    const list = demoArtists.filter(name => name.toLowerCase().includes(q.toLowerCase()));
    renderArtists(list.length ? list : demoArtists);
    if (!token) toast('Connect Spotify later to search the full Spotify catalog.');
  } catch (e) {
    const list = demoArtists.filter(name => name.toLowerCase().includes(q.toLowerCase()));
    renderArtists(list.length ? list : demoArtists);
    toast('Showing NAEVIS starter artists. Connect Spotify for the full catalog.');
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  $('#genreSearch').oninput = e => renderGenres(e.target.value);
  $('#clearGenres').onclick = () => { selectedGenres = []; localStorage.setItem('naevis_genres','[]'); renderGenres($('#genreSearch').value); updateMusicSummary(); };
  $('#saveMusic').onclick = () => { localStorage.setItem('naevis_genres', JSON.stringify(selectedGenres)); localStorage.setItem('naevis_artists', JSON.stringify(selectedArtists)); toast('Your sound was saved!'); location.hash = '#spotify'; };
